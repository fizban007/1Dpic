...
Environment::Environment(int* argc, char*** argv)
  ...
  try {
    m_conf_file.parse_file(m_args.conf_filename());
...
Environment::setup_domain(int num_nodes) {
  ...
  //   if (total_dims > 1)
  //     fmt::print(stderr, "Supplied rank dimensions don't match total number
  //     of ranks!");
...
Environment::setup_domain(int dimx, int dimy, int dimz) {
  ...
  Logger::print_debug("Domain created with dimensions {} x {} x {}", dimx, dimy, dimz);
  ...
  // m_comm->cartesian().all_gather(&m_domain_info.cart_pos.x, 1, domain_pos_x,
  //                                 1);
  // m_comm->cartesian().all_gather(&m_domain_info.cart_pos.y, 1, domain_pos_y,
  //                                 1);
  // m_comm->cartesian().all_gather(&m_domain_info.cart_pos.z, 1, domain_pos_z,
  //                                 1);
...
// void
// Environment::set_initial_condition(SimData& data, const Index& start, const Extent& extent) {}
...
//     Logger::print_debug_all("Rank {} Adding boundary condition at pos {}", m_domain_info.rank, pos);
//     m_bc.add_fieldBC(bc);
//   }
// }
...
void
Environment::setup_local_grid(Grid &local_grid, const Grid &super_grid, const DomainInfo& info) {
  local_grid = super_grid;
  auto& local_mesh = local_grid.mesh();
  const auto& super_mesh = super_grid.mesh();
  if (info.cart_dims[0] > 0) {
    local_mesh.dims[0] =
        super_mesh.reduced_dim(0) / info.cart_dims[0] + 2 * super_mesh.guard[0];
    local_mesh.sizes[0] /= info.cart_dims[0];
  }
  if (info.cart_dims[1] > 0) {
    local_mesh.dims[1] =
        super_mesh.reduced_dim(1) / info.cart_dims[1] + 2 * super_mesh.guard[1];
    local_mesh.sizes[1] /= info.cart_dims[1];
  }
  if (info.cart_dims[2] > 0) {
    local_mesh.dims[2] =
        super_mesh.reduced_dim(2) / info.cart_dims[2] + 2 * super_mesh.guard[2];
...
//   data.E.initialize([this, &data](int n, Scalar x1, Scalar x2, Scalar x3){
//       return this->initial_condition().E(n, x1, x2, x3, data.E.grid().mesh());
//     });

//   data.B.initialize([this, &data](int n, Scalar x1, Scalar x2, Scalar x3){
//       return this->initial_condition().B(n, x1, x2, x3, data.B.grid().mesh());
//     });
